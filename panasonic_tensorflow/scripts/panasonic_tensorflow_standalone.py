...
sys.path.insert(0, rospack.get_path('panasonic_tensorflow'))
...
class FaceTensorFlow:
  ...
  def __init__(self):
    ...
    # Get ROS parameters
    self._published_topic = rospy.get_param('published_topic', '/number_pedestrians')
    self._video_device = rospy.get_param('video_device', '/dev/panasonic_camera')

    self._graph_name  = rospy.get_param('graph_name', '/frozen_inference_graph_person.pb')
    self._label_name  = rospy.get_param('label_name', '/mscoco_label_map.pbtxt')
    self._num_classes = rospy.get_param('num_classes', 1)
    ...
    self._pub = rospy.Publisher(self._published_topic, UInt8, queue_size=1)
  ...
  def detect_faces(self):
    ...
    self.publish_pedestrian_count(np.squeeze(boxes), np.squeeze(scores), np.squeeze(classes).astype(np.int32))
